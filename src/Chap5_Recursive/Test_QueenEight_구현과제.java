...
class Stack4 {
	...
	public boolean push(Point x) throws OverflowGenericStackException {
		if(isFull()) {
			throw new OverflowGenericStackException("Error : Stack Full");
		}
		top++;
		return data.add(x);
	...
	public Point pop() throws EmptyGenericStackException {
		if(isEmpty())
			throw new EmptyGenericStackException("Error : Stack Empty");
...
public class Test_QueenEight_구현과제 {
	public static void EightQueen(int[][] d) {
		...
		while (true) {
			...
			//다음 줄에 배치 성공하면 또 다음줄 실패하면 pop해서 뒤로
			if(iy!=-1) {
	...
	public static boolean checkDiagSW(int[][] d, int cx, int cy) { // x++, y-- or x--, y++ where 0<= x,y <= 7
		//x+y=a
		//x가 7이 되거나 y가 0이 되거나
		//x가 움직이는건 7-x y가 움직이는 건 y
		//ex 5,2 5가 7이 되거나 2가 0이 되거나 
		
		int start = (d.length-cx-1)>cy?cy:(d.length-cx-1);
		int x=cx+start;
		int y=cy-start;
	...
	public static int nextMove(int[][] d, int row, int col) {// 현재 row, col에 대하여 이동할 col을 return
		//for문으로 check무브해서 return col
		for(int i=col+1;i<d.length;i++) {
