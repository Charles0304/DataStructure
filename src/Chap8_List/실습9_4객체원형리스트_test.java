...
class CircularList {
	...
	public int Delete(SimpleObject3 element, Comparator<SimpleObject3> cc) // delete the element
	{
		...
		q = current;
		while(q.link!=first) {
			if(cc.compare(q.link.data, element)==0) {
				q.link=q.link.link;
				return 1;
			}
			q=q.link;
	...
	public void Add(SimpleObject3 element, Comparator<SimpleObject3> cc) // 임의 값을 삽입할 때 리스트가 오름차순으로 정렬이 되도록 한다
	{
		...
		if(cc.compare(first.link.data, newNode.data)>0) {
			newNode.link = first.link;
			first.link=newNode;
			return;
		}
		while(q.link!=first) {
			if(cc.compare(q.link.data, newNode.data)>0) {
				newNode.link=q.link;
				q.link=newNode;
				return;
			}
	...
	void Merge(CircularList b) {
		 ...
		 */
		Comparator<SimpleObject3> cc = SimpleObject3.NO_ORDER;
		Node3 p = first.link;
