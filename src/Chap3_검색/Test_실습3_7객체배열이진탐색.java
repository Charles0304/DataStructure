		...
		if(f1.name==f2.name){
			if(f1.price==f2.price) {
				if(f1.expire==f2.expire) {
					return 0;
				}else return f1.expire.compareTo(f2.expire);
			}else return f1.price>f2.price?1:-1;
...
public class Test_실습3_7객체배열이진탐색 {
	...
	private static int binarySearch(Fruit4[] arr, Fruit4 newFruit4, Comparator<Fruit4> cc_name) {
		...
		while(pl<=pr) {
			mid = (pr+pl)/2;
			if(cc_name.compare(newFruit4, arr[mid])==0) return mid;
			else if(cc_name.compare(newFruit4, arr[mid])<0) pr=mid-1;
			else pl = mid +1;
		}
		return 0;
	}
